...
class ContactForm extends Component {
  ...
  handleChangeObject (e) {
    ...
    if (e.target.value === '') { this.setState({ object_err: 'Required Field' }) } else { this.setState({ object_err: 'Enter Valid Email' }) }
  }

  handleChangeRum (e) {
    this.setState({ rum: e.target.value })
    if (e.target.value === '') { this.setState({ rum_err: 'Required Field' }) } else { this.setState({ rum_err: '' }) }
  }

  handleChangeKvm (e) {
    this.setState({ kvm: e.target.value })
    if (e.target.value === '') { this.setState({ kvm_err: 'Required Field' }) } else { this.setState({ kvm_err: '' }) }
  }
  ...
  handleChangeCity (e) {
    ...
    if (e.target.value === '') { this.setState({ city_err: 'Required Field' }) } else { this.setState({ city_err: '' }) }
  }

  handleChangeContact (e) {
    this.setState({ contactperson: e.target.value })
    if (e.target.value === '') { this.setState({ contactperson_err: 'Required Field' }) } else { this.setState({ contactperson_err: '' }) }
  }

  handleChangePostCode (e) {
    this.setState({ postcode: e.target.value })
    if (e.target.value === '') { this.setState({ postcode_err: 'Required Field' }) } else { this.setState({ postcode_err: '' }) }
  }

  handleChangeAdress (e) {
    this.setState({ adress: e.target.value })
    if (e.target.value === '') { this.setState({ adress_err: 'Required Field' }) } else { this.setState({ adress_err: '' }) }
  }

  handleChangeWindow (e) {
    this.setState({ window: e.target.value })
    if (e.target.value === '') { this.setState({ window_err: 'Required Field' }) } else { this.setState({ window_err: '' }) }
  }
  ...
  handleSubmit1 () {
    if (this.state.object === '') { this.setState({ object_err: 'Required Field' }) }
    if (this.state.rum === '') { this.setState({ rum_err: 'Required Field' }) }
    if (this.state.window === '') { this.setState({ window_err: 'Required Field' }) }
    if (this.state.kvm === '') { this.setState({ kvm_err: 'Required Field' }) }
    if (this.state.postcode === '') { this.setState({ postcode_err: 'Required Field' }) }
    if (this.state.phone === '') { this.setState({ phone_err: 'Required Field' }) }
    if (this.state.contactperson === '') { this.setState({ contactperson_err: 'Required Field' }) }
    if (this.state.city === '') { this.setState({ city_err: 'Required Field' }) }
          ...
          <form method='post' id='contact-form'>
            <div className='row'>
              ...
              <div className='col-lg-12'>
                <div className='col-lg-12'>
                  <textarea name='comments' id='user-message' value={this.state.message} onChange={this.handleChangeMessage} rows='6' className='con-field' placeholder='Beskriv gärna dina frågar' />
